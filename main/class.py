...
#аналогично a.setmoney(number)-в скобочках указываете число: на сколько надо измеенить баланс, тоесть если вписать -10, то от счета отнимется 10 валюты,
#просто 10 - это +10 к балансу
...
#советую сначало объявлять a=User("name") и за тем уже n=Achievement(a.getname()) (так правельней, не предется в ручную отслеживать имя пользователя, так же не будет конфликта с неинициализированным пользователем)
